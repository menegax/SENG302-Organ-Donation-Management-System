...
public class Database implements Serializable {
    ...
    final transient private ScreenControl screenControl = ScreenControl.getScreenControl();

    private static final Searcher searcher = Searcher.getSearcher();
    ...
    public static void addUser(User user) {
        if (user != null) {
            if (user instanceof Patient) {
                ...
                try {
                    database.addAdministrator((Administrator) user, searcher);
                } catch (IllegalArgumentException e) {
     ...
     */
    private ArrayList<String[]> runQuery(String query, String[] params) throws SQLException {
    ...
    private String[][] getPreparedUpdatePatientQuery(Patient patient) {
        ...
        StringBuilder query = new StringBuilder(UPDATEPATIENTQUERYSTRING);
        //Query to add patient contact details to database
        query.append(";" + UPDATEPATIENTCONTACTQUERYSTRING);
        ...
        for (Medication medication : patient.getCurrentMedications()) {
            ...
            query.append(";" + UPDATEPATIENTMEDICATIONQUERYSTRING);
        }
        ...
        for (Disease disease : allDiseases) {
            ...
            query.append(";" + UPDATEPATIENTDISEASESQUERYSTRING);
        }
        ...
        for (PatientActionRecord record : patient.getUserActionsList()) {
            ...
            query.append(";" + UPDATEPATIENTLOGQUERYSTRING);
        }
        ...
        for (Procedure procedure : patient.getProcedures()) {
            ...
            query.append(";" + UPDATEPATIENTPROCEDURESQUERYSTRING);
        }
        query.append("; ");
        String[] packagedQuery = {query.toString()};
     ...
     */
    private boolean nhiInDatabase(String nhi) {
    ...
    private boolean patientInDatabase(Patient patient) throws SQLException {
        ...
        return runQuery(query, param).size() > 0;
    }
     ...
     */
    private boolean inDatabase(Object object) {
        try {
            if (object instanceof Patient) {
    ...
    private Patient parsePatient(String[] attr) {
        String nhi = attr[0];
        String fName = attr[1];
        ArrayList<String> mNames = new ArrayList<>(Arrays.asList(attr[2].split(" ")));
        String lName = attr[3];
        ...
        LocalDateTime death = null;
        if (attr[7] != null) {
            death = LocalDateTime.parse(attr[7]);
        }
        GlobalEnums.BirthGender gender;
        switch (String.valueOf(attr[8])) {
            case "M":
...
//        return meds;
    ...
    private boolean deletePatient(Patient patient, Searcher searcher) {
        ...
        while (counter < tables.length - 1) {
            query.append("DELETE FROM ").append(tables[counter]).append(" WHERE Patient = ?;");
            params[counter] = nhi;
            counter += 1;
        }
        query.append("DELETE FROM ").append(tables[counter]).append(" WHERE Nhi = ?");
        params[counter] = nhi;
        try {
            runQuery(query.toString(), params);
     ...
     */
    private boolean administratorInDb(String username) {
     ...
     */
    private Patient getPatientByNhi(String nhi) {
            ...
            if (insertCount == 4000) {
                ...
                while (queryCount < queries.length) {
                    ...
                    if (!query.equals("")) {
                        ...
                        finalQuery.append(addQueryEnd(query, queryCount)).append(";");
                        finalParams = ArrayUtils.addAll(finalParams, params[queryCount]);
                    }
                    queryCount += 1;
                }
                try {
                    runQuery(finalQuery.toString(), finalParams);
    ...
    private boolean updateAllClinicians() {
        ...
        for (Clinician clinician : getClinicians()) {
            if (clinician.getChanged()) {
                query.append(UPDATECLINICIANQUERYSTRING + ";");
    ...
    private boolean updateAllAdministrators() {
        ...
        for (Administrator admin : getAdministrators()) {
            if (admin.getChanged()) {
                query.append(UPDATEADMINQUERYSTRING + ";");
...
//            saveToDiskWaitlist();
    ...
    private Set<Clinician> getClinicians() {
        return clinicians;
    }
     ...
     */
    private boolean staffIDInDatabase(int staffID) {
        String query = "SELECT * FROM tblClinicians WHERE StaffID = ?";
        String[] param = {String.valueOf(staffID)};
        try {
